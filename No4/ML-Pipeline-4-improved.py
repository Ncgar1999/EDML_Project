"""
Improved ML Pipeline for Student Grade Prediction
This script predicts student final grades (G3) using a regression approach.

Key improvements:
- Converted from classification to regression
- Added proper data validation and error handling
- Implemented better preprocessing with scaling and encoding
- Added cross-validation and hyperparameter tuning
- Included comprehensive evaluation metrics
...
def main():
    ...
    try:
        ...
        # Note: Consider whether to include G1 and G2 based on your prediction objective
        # If predicting without knowing intermediate grades, exclude G1 and G2
        exclude_columns = ['G3']  # Add 'G1', 'G2' if avoiding data leakage
        ...
        # Numerical features
        num_features = preprocessor_fitted.named_transformers_['num']
        ...
        logger.info("SUMMARY")
        logger.info("="*50)
        logger.info(f"Best Model: Random Forest Regressor")
        logger.info(f"Test R² Score: {test_metrics['r2']:.4f}")
        logger.info(f"Test RMSE: {test_metrics['rmse']:.4f}")
        logger.info(f"Test MAE: {test_metrics['mae']:.4f}")
        
        if test_metrics['r2'] > 0.6:
            logger.info("✓ Model shows good predictive performance")
        else:
            logger.info("⚠ Model performance could be improved")
            logger.info("Consider: feature engineering, different algorithms, or more data")
