"""
Final Corrected ML Pipeline for COMPAS Recidivism Prediction

This script addresses ALL issues found in the original ML-Pipeline-2.py:
- Fixes data leakage in preprocessing
- Removes target leakage (decile_score predicts score_text directly)
...
def load_and_engineer_features():
    ...
    print("NOTE: Excluded 'decile_score' to avoid target leakage (it directly determines score_text)")
    ...
    if 'dob' in data.columns:
        ...
        # Calculate age from birth year (using 2013-2014 as reference based on screening dates)
        data['age_from_birth'] = 2013 - data['birth_year']  # More accurate based on screening date range
    ...
    # Days before screening categories
    if 'days_b_screening_arrest' in data.columns:
        data['screening_delay'] = pd.cut(data['days_b_screening_arrest'], 
                                       bins=[-np.inf, 0, 30, 365, np.inf], 
                                       labels=['same_day', 'within_month', 'within_year', 'over_year'])
...
def preprocess_features(X_train, X_test):
    ...
    numerical_cols = X_train_processed.select_dtypes(exclude=['object', 'category']).columns
...
def analyze_bias(X_test, y_test, y_pred, label_encoders):
    ...
    if 'race' in X_test_decoded.columns:
        ...
        for race in X_test_decoded['race'].unique():
            ...
            if mask.sum() > 0:
                ...
                race_f1 = f1_score(y_test[mask], y_pred[mask], average='macro')
                print(f"{race}: Accuracy={race_accuracy:.4f}, F1={race_f1:.4f} (n={mask.sum()})")
    
    # Analyze by sex (if available)
    if 'sex' in X_test_decoded.columns:
        print("\nPrediction accuracy by sex:")
        for sex in X_test_decoded['sex'].unique():
            mask = X_test_decoded['sex'] == sex
            if mask.sum() > 0:
                sex_accuracy = accuracy_score(y_test[mask], y_pred[mask])
                sex_f1 = f1_score(y_test[mask], y_pred[mask], average='macro')
                print(f"{sex}: Accuracy={sex_accuracy:.4f}, F1={sex_f1:.4f} (n={mask.sum()})")
...
def train_and_evaluate():
    ...
    # Bias analysis
    analyze_bias(X_test_processed, y_test, y_pred, label_encoders)
    ...
    print("✓ Target leakage removed (excluded decile_score)")
    ...
    print("✓ Bias analysis included for fairness assessment")
    
    return clf, accuracy, macro_f1

if __name__ == "__main__":
    print("FINAL CORRECTED COMPAS RECIDIVISM PREDICTION PIPELINE")
    print("="*70)
    model, accuracy, macro_f1 = train_and_evaluate()
    print(f"\nFinal Results: Accuracy={accuracy:.4f}, Macro F1={macro_f1:.4f}")
    print("\nNote: Performance is now realistic without target leakage!")